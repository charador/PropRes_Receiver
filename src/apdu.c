...
BACNET_WRITE_PROPERTY_DATA global_wp_data;
...
/*added set function for wp execute handler*/
static wp_execute_fun function_handler;

void wp_set_execute_fun(wp_execute_fun function) {
    function_handler = function;
    fprintf(stderr, "WP execute function is linked!\n");
}
...
bool WPExecuteCommand(uint8_t * random) {
    ...
    if (match) {
        ...
        //        if (Analog_Value_Write_Property(&global_wp_data)){
//        if (ExecuteWP(&global_wp_data)) {
                    if (function_handler(&global_wp_data)){
...
void apdu_handler(
    ...
    bool fin_bit = false;
    uint8_t random[8] = {0};
    bool status = false; // if wp is succeed
        ...
        switch (apdu[0] & 0xF0) {
            ...
            case PDU_TYPE_SIMPLE_ACK:
                ...
                switch (service_choice) {
                    ...
                    case SERVICE_CONFIRMED_DELETE_OBJECT:
                    case SERVICE_CONFIRMED_WRITE_PROPERTY:
                        /*added for WP challenge handler*/
                        if (apdu_len > 3) {
                            if (apdu[3] == 0x01) {
                                fin_bit = 1;
                            } else {
                            ...
                            if (fin_bit) {
                                // it is a FIN WP challenge, copy random number
                                for (int i = 0; i < 8; ++i) {
                                    random[i] = apdu[4 + i];
                                }

                                status = WPExecuteCommand(random);
                                if (status) {
                                    //mark global_WP_data as not in use
                                    global_wp_data.inuse = false;
                                    // send ACK back
                                    WP_Ack(src, invoke_id);
                                }
                                //tsm_free_invoke_id(invoke_id);
                                break;
                            } else {
                                fprintf(stderr, "Receiver: Received a NONFIN challenge!\n");
                            }
                        } else {// when a normal WP ACK is received, call general ack function
